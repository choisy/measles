...
diff %>% gather(key = infectives, value = probability, X1:X10)  %>%
  ...
  labs(x = "vaccine coverage", y = "probability of an epidemic")
```
...
sim3City <- seirNCity(num = 3, N = c(1e6, 1e6, 1e6), 
                      ...
                      fM = matrix(c(0.098, 0.001, 0.001, 0.1, 0.9, 0, 0.1, 0, 0.9), 
                                 nrow = 3, ncol = 3, byrow = TRUE), tf = 150)
```
...
## Approximation of the Population Network
...
```{r}
set.seed(10)
sseir1 <- seir(N = 1e6, I0 = 1, p = 0.5, beta = 5, sigma = 1/7, gamma = 1/7, tf = 100)
...
The prevalence curves for the single population deterministic and stochastic models are consistent. 
...
```{r}
y1 <- smooth.spline(x = sim6$p, y = sim6$epi_proba, df = 30)
...
nCityApprox <- function(n = 2, N = c(1e6, 1e6), v = c(0.95, 0.95), Y0 = c(1, 0),
                          rho = matrix(c(0, 0.01, 0.01, 0), nrow = 2, ncol = 2, byrow = TRUE), 
                          tf = 365) {
 ...
 # Generate prevalence curves for infected populations
  ...
  for(t in 1:tf) {
    for(i in which(I[t, ] == 0)) {
      ...
      if (runif(1, 0, 1) < prob) 
        I[, i] <- c(rep(0, t), dseir(N = N[i], Y0 = 1, v = v[i], tf = tf - t)$Y)
    }
  }
  I <- as.data.frame(I) %>% mutate("time" = seq(1:(tf+1))) %>% select("time", dplyr::everything())
...
sim15 <- nCityApprox(n = 2, N = c(1e6, 1e6), v = c(0.95, 0.5), 
                     Y0 = c(0, 1), tf = 400, 
                     rho = matrix(c(0, 0.02, 0.1, 0), nrow = 2, ncol = 2, byrow = TRUE))
```
...
sim16 <- nCityApprox(n = 3, N = c(1e6, 1e6, 1e6), 
                       ...
                       rho = matrix(c(0, 0.001, 0.001, 0.1, 0, 0, 0.1, 0, 0), 
                                 nrow = 3, ncol = 3, byrow = TRUE), tf = 700)
```
...
The following approximation of the exact stochastic model attempts to account for the lag effects that were observed in the first approximate model by coupling the deterministic populations along the way. 
...
```{r}
nCityApprox2 <- function(n = 2, N = c(1e6, 1e6), v = c(0.95, 0.95), Y0 = c(1, 0),
                          rho = matrix(c(0, 0.01, 0.01, 0), nrow = 2, ncol = 2, byrow = TRUE), 
                          tf = 365) {
