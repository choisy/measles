...
prov_coloc <- reshape2::melt(coloc_mat) %>%
  setNames(c("polygon1_id", "polygon2_id", "link_value")) %>%
  dplyr::right_join(select(fb_pop, polygon1_id, province)) %>%
...
The following code summarizes the province-level population and geographical data. 
```{r warning = FALSE}
...
Let's map the intracity $R_0$ values for the provinces:
The following pr have intercityR0 values greater than 1:
```{r}
